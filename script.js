...
let configuracionSecreta = null;

import('./config.secret.js')
    .then(modulo => {
        configuracionSecreta = modulo;
    })
    .catch(() => {
        console.warn('No se encontro config.secret.js. La publicacion administrativa esta desactivada.');
    });
...
window.procesarLogin = function(event) {
    ...
    if (!configuracionSecreta?.ADMIN_PASSWORD) {
        alert('No se encontro la configuracion privada del administrador.');
        return;
    }

    if (passwordInput === configuracionSecreta.ADMIN_PASSWORD) {
        isAdmin = true;
        cerrarModalLogin();
        abrirModalPublicar();
    } else {
